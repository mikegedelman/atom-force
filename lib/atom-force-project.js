...
export default class SfProject extends EventEmitter {
  ...
  getPrimaryConnection() {
    return this._getPrimaryData().then((oauth) => new SfConnection(oauth));
  }

  // Look in user's root directory for projectFilePath (hardcoded as [root]/.atomforce right now)
  _getPrimaryData() {
    if (this.projectData) {
      return Promise.resolve(this.projectData.primary.oauth);
    }
  ...
  setPrimaryData(oauth) {
    this.projectData = { primary: { name: 'Primary', oauth }, others: [] };
    this._write();
  }
