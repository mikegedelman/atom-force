...
export default class SfConnection extends EventEmitter {
  ...
  deployMetadata(paths) {
    ...
    paths.forEach(filePath => { // create a bunch of promises, but don't .then until below
      ...
      filePromises.push(util.readFilePromise(filePath).then(data => {
        log.info(`read ${filePath}`);
        zip.folder(folder).file(basename, data);
      }));
      filePromises.push(util.readFilePromise(`${filePath}-meta.xml`).then(data => {
        log.info(`read ${filePath}`);
        zip.folder(folder).file(`${basename}-meta.xml`, data);
      }));
    ...
    Promise.all(filePromises).then(() => {
      log.info('starting async zip generation.');
      // nforce-metadata would just turn our buffer into base64 anyway
      return zip.generateAsync({ type: 'base64' });
    })
    .then(zipFile => {
      log.info('got base64 from generateAsync.');
      ...
      deployPromise.error(err => this.emit('saveError', err));
